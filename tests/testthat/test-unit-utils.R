...
test_that("T-UTIL-08b every row still invalid after repair: a geometry error, also for several rows", {
  square <- sf::st_polygon(list(rbind(c(0, 0), c(1, 0), c(1, 1), c(0, 1), c(0, 0))))
  x <- sf::st_sf(
    id = c("a", "b", "c"),
    geometry = sf::st_sfc(square, square + 2, square + 4, crs = 5070)
  )
  testthat::local_mocked_bindings(
    st_is_valid = function(x, ...) rep(FALSE, length(sf::st_geometry(x))),
    .package = "sf"
  )
  err <- tryCatch(suppressWarnings(.repair_geometry(x)), error = function(e) e)
  expect_s3_class(err, "catchmentACS_error_geometry")
  expect_match(conditionMessage(err), "all 3 rows", fixed = TRUE)
})
