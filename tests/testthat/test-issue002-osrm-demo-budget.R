...
#
# Public surface tests for the Issue 002 fix:
...
test_that("ISSUE002-08 emit captured by short-suffix 'demo_budget_protected'", {
  # The notice is given once per session, and earlier tests may have given
  # it already. Forget that for this test and put the record back afterwards.
  freq_env <- get("message_freq_env", asNamespace("rlang"))
  notice_id <- "catchmentACS_demo_budget_protected"
  had_record <- exists(notice_id, envir = freq_env, inherits = FALSE)
  old_record <- if (had_record) get(notice_id, envir = freq_env, inherits = FALSE)
  if (had_record) rm(list = notice_id, envir = freq_env)
  withr::defer({
    if (exists(notice_id, envir = freq_env, inherits = FALSE)) {
      rm(list = notice_id, envir = freq_env)
    }
    if (had_record) assign(notice_id, old_record, envir = freq_env)
  })
  ...
  # The way to ask for more detail, in the form each function takes.
  expect_match(cap$message[[1L]], "cacs_isochrone(res = 70L)", fixed = TRUE)
  expect_match(cap$message[[1L]], "iso_args = list(res = 70L)", fixed = TRUE)
})
