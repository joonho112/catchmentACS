# ============================================================================
# Integration tests for cacs_isochrone() end-to-end.
# 3 cases:
...
# T19-13 (HTTP 429 abort) is covered at the helper level in
# test-integration-isochrone-{osrm,ors}.R.
#
...
# is bypassed where needed).
# ============================================================================


# ---- Isolated cache dir per test (as in the isochrone cache test) ----------
...
test_that("T19-09a osm_snapshot_date = NULL records 'unknown' / 'unknown_best_effort'", {
  ...
  .with_iso_cache_dir({
    ...
    # A best-effort lookup failure yields the "unknown"
...
test_that("T19-09b osm_snapshot_date = Date records 'user_supplied' + explicit date", {
  ...
  .with_iso_cache_dir({
    ...
    # Explicit Date -> status = "user_supplied", date = ISO chr.
...
test_that("T19-09c NULL vs Date produce distinct cache keys (separate key spaces)", {
  ...
  .with_iso_cache_dir({
    ...
    # The "unknown" sentinel forms a *separate* key space so a
  ...
  .with_iso_cache_dir({
    ...
    out <- suppressMessages(withCallingHandlers(
      ...
      catchmentACS_warning_runtime = function(w) {
        if (grepl("drive-time areas?\\s+from\\s+\\S+\\s+(is|are)\\s+empty", conditionMessage(w))) {
          warn_seen <<- TRUE
        }
...
# ============================================================================
# T19-15 was planned as 10 sites = 7 successful + 2 empty + 1 retry-exhaustion.
...
# is the "1 retry-recovered" case named above, and it exercises the
...
test_that("T19-15 mixed batch (7 ok + 2 empty + 1 retry) returns 10 rows with correct provenance", {
  ...
  .with_iso_cache_dir({
    ...
    expect_true(any(grepl("drive-time areas?\\s+from\\s+\\S+\\s+(is|are)\\s+empty", warn_msgs)))   # W-09 fired
    ...
    # ---- 16-col canonical schema invariance --------------------------------
    expect_named(out, c(
...
# ============================================================================
# T19-16 (v0.2): verbose=TRUE emits >= n_sites progress tick events
# ============================================================================
# Asserts that verbose = TRUE
