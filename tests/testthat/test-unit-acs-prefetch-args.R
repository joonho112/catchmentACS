# ============================================================================
# Unit tests for R/acs-prefetch.R: argument checks and variable resolution.
#
# Core cases (T20-01..T20-08 + T20-10):
...
#
# A 10th sanity test confirms the frozen argument signature so a future
...
# ===========================================================================
# T20-00 Signature: 10 args in canonical order (frozen; v0.2 widened
# from 9 to 10 to add `drop_water_tracts` for the BUG-001 hybrid filter).
# ===========================================================================
...
test_that("cacs_acs_prefetch() formal signature is frozen at 10 args (v0.2)", {
  ...
  # `drop_water_tracts` defaults to TRUE (v0.2 BUG-001 auto-filter on
...
# ===========================================================================
# T20-02 state vector → schema abort + one-state-per-call hint
# ===========================================================================

test_that("T20-02 state = c('AL','MS') aborts with a one-state-per-call hint", {
  .with_fake_census_key({
    err <- expect_error(
  ...
  # cacs_run() also takes a single state, so the message does not suggest it.
  expect_false(grepl("cacs_run", conditionMessage(err), fixed = TRUE))
})
...
# ===========================================================================
# T20-04 survey = 'acs1' → schema abort, only acs5 is accepted
# ===========================================================================

test_that("T20-04 survey = 'acs1' aborts and names acs5 as the accepted survey", {
  .with_fake_census_key({
    err <- expect_error(
  ...
  expect_false(grepl("v1.1", conditionMessage(err), fixed = TRUE))
})
...
test_that("T20-05c years outside the accepted ACS5 range abort", {
...
test_that("T20-06 variables = NULL falls back to cacs_acs_default_vars (14)", {
  .with_fake_census_key({
    ...
    expect_s3_class(out, "sf")
    # The provenance hooks are folded into canonical provenance.
...
test_that("T20-06d codebook endpoint-not-found is surfaced as schema tier", {
  .with_fake_census_key({
    ...
    expect_error(
      ...
      class = "catchmentACS_error_schema",
      regexp = "list of ACS5 variables for"
    )
  })
})
...
test_that("T20-10 partial-invalid variables warn (W-20-04) + skip + proceed", {
  .with_fake_census_key({
    ...
    expect_s3_class(out, "sf")
    # Provenance hooks are folded into canonical provenance.
    prov <- attr(out, "cacs_provenance")
    expect_false(is.null(prov))
    expect_identical(prov, attr(out, "cacs_acs_provenance"))
