...
#
# Regression test on made-up inputs: the 540 rows for area weighting must match
# the saved output in inst/extdata/legacy_2025_golden_output.rds within
# 1e-6 (estimate), 1e-5 (moe), and 1e-9 (weight_sum).
#
...
# activate it.
#
# The saved output was produced by running `cacs_run()` on the 20 made-up
# sites, the circles around them, and the made-up ACS values in inst/extdata
# (see inst/extdata/README.md). The test therefore checks that the same inputs
# still give the same output and the same schema; it does not compare the
# package with an earlier analysis.
#
...
# block (Step 6 below) locks the 27 -> 20 ASYMMETRY explicitly so a
...
#
# On failure, follow this five-step drift triage:
#   Step 1: classify origin (algorithm bug / toolchain drift / golden
#           bit-rot / intended behavior change)
#   Step 2: collect evidence (git log on R/, sf_extSoftVersion(), the
#           fixture sha256)
...
.c10_area_band_reference <- function() {
  tibble::tibble(
    site_id = c("AL_ANCHOR_03", "AL_ANCHOR_02", "AL_ANCHOR_01"),
    band = c("narrow", "middle", "wide"),
    area_band_pct = c(6, 16, 60)
  )
}


test_that("C10 areas of the three-site fixture stay within their area bands", {
  fixture <- load_replay_fixture("031_3site_anchors")
  iso <- fixture$iso_sf_v2
  ref <- .c10_area_band_reference() |>
  ...
  wide_band_site <- cmp[cmp$site_id == "AL_ANCHOR_01", ]
  testthat::expect_equal(wide_band_site$band, "wide")
  testthat::expect_gt(wide_band_site$pct_diff, 30)
  testthat::expect_lte(wide_band_site$pct_diff, 60)
  testthat::expect_equal(wide_band_site$seam_jaccard, 1)
})


test_that("cacs_run() reproduces the saved regression output (area, 540 rows)", {
  skip_on_cran()
  ...
  testthat::expect_equal(nrow(golden), 1080L)  # 540 area + 540 population rows
  ...
  # v0.1 area-only subset of the full golden
  ...
  # ------------------------------------------------------------------------
  # Step 4: per-column tolerance assertion
  # ------------------------------------------------------------------------
  ...
  # ------------------------------------------------------------------------
  # Step 6 (v0.2.0 re-baseline): schema growth assertion
  #
  ...
  # Rate rows: NA on `n_tracts`, populated on the two new count cols.
  ...
  if (nrow(rate_rows) > 0L) {
    ...
    # The two count columns are populated when the carrier resolved
  ...
  # Source rows: NA on both count cols (preserves the v0.1 invariant that
