...
test_that("C03-RSCRIPT-03 water condition is visible in Rscript subprocess", {
  ...
  writeLines(c(
    ...
    "library(sf)",
    "library(tibble)",
    # deparse() writes the path as an R string with its backslashes escaped,
    # so the script parses on Windows too.
    sprintf("pkg_path <- %s", deparse(pkg_path)),
    ...
    "  classes = 'water_tract_filter')",
    sprintf("saveRDS(out, %s)", deparse(out_file))
  ), script)
