...
#   - helper_mock_osrm_response() — osrm::osrmIsochrone() mock
...
#' passes the schema check of \code{cacs_acs_prefetch()}. The mock is
...
#'   \code{estimate = NA} to simulate Bureau suppression (used by the
#'   integration tests).
#' @param moe_sentinel_pct Numeric in [0, 1]; fraction of rows with
#'   \code{moe = -555555555} to simulate Bureau MOE sentinels.
#' @return An sf tibble with the 6 canonical columns + EPSG:4269.
helper_mock_acs_long_sf <- function(state            = "AL",
  ...
  # Inject suppression / sentinels for the ACS prefetch integration tests
...
# helper_mock_osrm_response()   — OSRM mock fixture
# helper_mock_ors_response()    — ORS mock fixture
# helper_with_mock_dir()        — httptest2::with_mock_dir() wrapper
