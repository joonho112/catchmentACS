...
|---|---|---|
| `T-P2-SCHEMA-` | Schema locks: canonical isochrone/long schemas, condition classes, preflight issue shape, replay upgrade checks. | `test-phase2-schema-locks.R` |
| `T-P2-GOLDEN-` | Golden compatibility checks against v0.2 weighted-output baselines. | `test-phase2-schema-locks.R` |
| `T-P2-COVERAGE-` | Branch coverage for validator and rings-helper edge cases. | `test-phase2-schema-locks.R` |
| `T-P2-PIPE-` | Pipeline integration checks for `cacs_run()` bypass paths and topology propagation. | `test-integration-phase2-pipeline.R` |
...
| `T-UF1-RINGS-` | Annulus-to-cumulative conversion helper tests. | `test-unit-cacs-rings-to-cumulative-helper.R` |
| `T-UF1-VAL-` | Cumulative-only validator tests and annulus hard-rejection behavior. | `test-unit-cumulative-rings-validator.R` |
| `T-P3-CACHE-FP-` | Cache fingerprint sidecar contract. | `test-unit-cache-fingerprint.R` |
| `T-P3-CACHE-ISO-` | ACS `acs_test/` namespace isolation. | `test-unit-cache-isolation.R` |
| `T-P3-CACHE-KEY-` | Cache-key payload dimensions and content-aware invalidation. | `test-unit-cache-key-payload.R` |
...
| `P4-SSI-` | SSI formula correction and old-formula exclusion. | `test-bug012-ssi-formula.R` |
| `C09-` | C-09 carrier-missing closure and side-effect checks. | `test-c09-carrier-missing-derive-rates.R` |
| `P4-GOLD-` | Hand-computed five-rate golden tests. | `test-golden-5rate-hand-computed.R` |
| `P4-CANON-` | Sanctioned rate catalogue and audit-bound contract. | `test-integration-rate-formulas-canonical.R` |
| `P4-NONA-` | Carrier-complete default-path no-NA contract. | `test-integration-derive-rates-no-na.R` |
| `P4-PROV-` | Rate provenance and audit metadata. | `test-phase4-rate-provenance-audit.R` |
| `P4-SIDE-` | Side-effect tests proving SSI/carrier changes do not mutate unrelated rates. | `test-side-effect-other-rates-unchanged.R` |
