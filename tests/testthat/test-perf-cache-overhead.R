...
# meaningful on a developer machine: CRAN machines and shared CI runners carry
# other load, and `covr` coverage instrumentation slows execution unpredictably
# (often 2-10x), which makes the tight micro-budgets (e.g. 100us per counter
# bump) fail there for reasons unrelated to the package. The time checks
# therefore skip on CRAN, on CI, and under covr, and run only where NOT_CRAN is
# "true" outside CI and covr. The other checks in these tests come before the
# skips and run everywhere.
