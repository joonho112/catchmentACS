# ============================================================================
# Integration tests for cacs_run(): 5-call sequence + output dispatch +
# attribute carry-through.
#
# cacs_run() checks its arguments, sets the execution path, validates the
# `*_args` lists, and calls the 5 sub-functions in order. It then dispatches
# the output in 3 modes (`"long"` / `"list_column"` / `"both"`), using the
# `.pivot_to_list_column()` helper (8-col list-column schema), and attaches
# the 4 run-level attributes (`cacs_schema_version`, `cacs_run_provenance`
# with at least 16 fields, the phase-keyed `cacs_run_warnings` list, and
# sub-call provenance preservation).
#
# Tests in this file therefore inspect:
#   - sub-call invocation count (verifies the bypass logic)
#   - sub-call invocation order (verifies the explicit phase sequence)
#   - warning capture into `warning_log$entries` (verifies warning aggregation)
#   - partial-success NA rows / E-24-15 abort
#   - 3-mode output dispatch + 16-field provenance + warning attribute attach
#
...
# returns a fixture-shaped value. This isolates the orchestration layer of
# cacs_run() from sub-function correctness (those are exercised by
...
#   T24-02  4-call-A bypass (precomputed iso)   — isochrone not called, M-24-03
#   T24-03  4-call-B bypass (acs supplied)      — prefetch not called, M-24-02
#   T24-04  3-call full bypass                  — isochrone + prefetch skipped
...
#   T24-10  output = "list_column" 8-col format — exact column set + order
...
#   T24-12  cacs_run_provenance 16 fields       — required field names
#   T24-13  cacs_run_warnings phase-keyed       — 6 named phase slots
# ============================================================================


# ---- Shared fixtures ------------------------------------------------------
#
# `.run_sites_fix(n)` builds a valid `sites` tibble (n rows).
# `.run_iso_fix(...)` builds a minimal 16-col canonical isochrone sf so the
#   `iso_summary` group_by in `cacs_run()` finds the required columns
#   (`site_id`, `drive_time_min`, `isochrone_empty`, `failure_reason`).
# `.run_acs_fix()` builds a minimal 6-col canonical ACS sf (only the columns
#   `cacs_intersect_weight()` validates).
# `.run_weighted_fix()` builds a minimal tibble like the output of
#   `cacs_intersect_weight()`, with the `cacs_aggregation_carriers`
#   attribute, so the `cacs_propagate_moe()` / `cacs_derive_rates()` stubs
#   do not need to re-create the carrier contract (they are mocked anyway).
...
# Build a canonical long tibble (the cacs_intersect_weight() output shape)
# for the cacs_propagate_moe() -> cacs_derive_rates() stubs to return.
# Includes the byte-identical 18 mandatory columns the orchestrator's
# NA-propagation logic references.
...
test_that("T24-01 5-call full path invokes all 5 sub-functions in order", {
  ...
  stub_iw  <- function(iso_sf, acs_sf, bg_pop_sf = NULL,
    ...
    # Mimic the cacs_intersect_weight() carrier attribute so the
    # cacs_derive_rates() stub can pretend the contract held.
  ...
  # cacs_run() returns the long tibble; no operator-class abort.
...
# ===========================================================================
# T24-02  4-call-A bypass — precomputed isochrones, cacs_isochrone() skipped
# ===========================================================================
...
test_that("T24-02 4-call-A bypass (precomputed iso) skips cacs_isochrone()", {
  ...
  # With verbose = TRUE, a progress message says the supplied areas are used.
  expect_message(
    suppressWarnings(
      cacs_run(sites = sites, state = "AL",
               precomputed_isochrones = iso_in, verbose = TRUE)
    ),
    "Using the drive-time areas given in",
...
# ===========================================================================
# T24-03  4-call-B bypass — user-supplied acs, cacs_acs_prefetch() skipped
# ===========================================================================
...
test_that("T24-03 4-call-B bypass (acs supplied) skips cacs_acs_prefetch()", {
  ...
  # With verbose = TRUE, a progress message says the supplied ACS data are used.
  expect_message(
    suppressWarnings(
      cacs_run(sites = sites, state = "AL", acs = acs_in, verbose = TRUE)
    ),
    "Using the ACS data given in",
    class = "catchmentACS_message_progress"
  )
})
...
# ===========================================================================
# T24-04  3-call full bypass — both precomputed; isochrone + prefetch skipped
# ===========================================================================
...
test_that("T24-05 partial-success: 1 failed pair triggers W-24-02 + NA propagation", {
  ...
  intersect_input <- NULL  # capture what reaches cacs_intersect_weight()
...
# ===========================================================================
#
# The MOE stub emits a cli_warn; .call_with_capture() must capture it into
...
test_that("T24-07 catastrophic all-fail aborts with E-24-15 (operator)", {
  ...
  expect_match(conditionMessage(err), "for any of the", fixed = TRUE)
  expect_match(conditionMessage(err), "Routing gave no area", fixed = TRUE)
})
...
test_that(".build_na_propagation_rows preserves template column order + types", {
  ...
  # 4-field sentinel contract of the NA rows.
...
# ===========================================================================
# T24-09..13  Output dispatch + attribute carry-through
# ===========================================================================
#
...
test_that("T24-09 output = 'long' returns the long tibble + carries attributes", {
  ...
  # Default "long" mode returns the cacs_derive_rates() long tibble as-is
  # (no nesting).
  expect_s3_class(out, "tbl_df")
...
test_that("T24-10 output = 'list_column' produces the Sec. 12.3.2 8-col schema", {
  ...
  # 8-col list-column schema (exact column set + order).
  expect_s3_class(out, "tbl_df")
  expect_equal(
    colnames(out),
