test_that("P6-LOCK-ISO-01 mocked docker path stays below 15 seconds per site", {
  ...
  with_test_cache({
    ...
    # A wall-clock budget depends on the load of the machine, so it is checked
    # only where NOT_CRAN is "true", after the other expectations.
    skip_on_cran()
...
test_that("P6-LOCK-ISO-02 explicit res=70 remains inside mocked 15s/site lock", {
  ...
  with_test_cache({
    ...
    # Wall-clock budget last, only where NOT_CRAN is "true" (see P6-LOCK-ISO-01).
    skip_on_cran()
