...
#
# Unit tests for `.cacs_osrm_resolve_demo_res()`
...
test_that("STEP23-07 explicit res=30L + demo + protect → 30L passthrough (no emit)", {
  # A user who gives 30L already stays within the demo budget, so no notice.
...
test_that("STEP23-09 invalid options_protect aborts", {
  expect_error(.helper_call(NULL, "demo", NA),
               "catchmentACS.osrm_demo_budget_protect must be TRUE or FALSE", fixed = TRUE)
