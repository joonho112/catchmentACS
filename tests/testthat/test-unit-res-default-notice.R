# Unit tests for the v0.3 OSRM res default and once-per-session notice.
...
test_that("res default notice helper emits classed message with recovery text", {
  .with_fresh_res_notice_once({
    ...
    # How to change the value, without the history of development versions.
    expect_match(conditionMessage(msg), "to choose it", fixed = TRUE)
    # Both ways to give res: to cacs_isochrone() and through cacs_run().
    expect_match(conditionMessage(msg), "iso_args = list(res = ", fixed = TRUE)
    expect_false(grepl("v0.3.0", conditionMessage(msg), fixed = TRUE))
  })
})
