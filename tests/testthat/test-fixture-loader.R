...
test_that("replay fixture paths resolve for all bundled fixtures", {
  for (nm in shipped_replay_fixture_names()) {
...
test_that("replay fixture paths resolve for the other fixtures when present", {
  for (nm in setdiff(replay_fixture_names(), shipped_replay_fixture_names())) {
    skip_if_no_replay_fixture(nm)
    path <- replay_fixture_path(nm)
    expect_true(nzchar(path), info = nm)
    expect_true(file.exists(path), info = nm)
    expect_match(basename(path), paste0("^fixture_", nm, "\\.rds$"))
...
test_that("load_replay_fixture returns expected object classes for 031", {
...
expect_replay_provenance <- function(fx) {
...
test_that("replay fixtures carry provenance metadata", {
  for (nm in shipped_replay_fixture_names()) {
    expect_replay_provenance(load_replay_fixture(nm))
  }
})

test_that("the other replay fixtures carry provenance metadata when present", {
  for (nm in setdiff(replay_fixture_names(), shipped_replay_fixture_names())) {
    expect_replay_provenance(load_replay_fixture(nm))
  }
})

test_that("skip guard succeeds when the bundled replay fixtures are installed", {
  expect_invisible(skip_if_no_replay_fixtures(shipped_replay_fixture_names()))
})
