# ============================================================================
# Known-answer tests for the per-site loop body of R/intersect-weight.R.
#
# Coverage (per-site loop body):
#
...
#             internal closure 3 times and asserting independence.
#
# Strategy: the per-site loop returns an INTERMEDIATE result (the per-tract
# long tibble). The tests exercise `.intersect_one_site()` DIRECTLY (it is
# internal but reachable via `:::`). The fixtures are pre-projected to
# EPSG:5070 (per inst/testdata convention), which is exactly what the
# closure expects.
# ============================================================================
...
# sf_use_s2(FALSE) for planar geometry math and (b) precompute `tract_area_m2`
# on the ACS sf, as cacs_intersect_weight() does before its per-site loop.
...
# ============================================================================
# Batch integration smoke - the full public entry returns a valid
# 20-col canonical long output.
#
...
test_that("Step 5.4 public entry returns canonical long output after full algorithm", {
  ...
  # 14-col iso schema + MULTIPOLYGON-cast acs schema. cacs_intersect_weight()
  # then transforms back to EPSG:5070 internally.
  iso_4326 <- sf::st_sf(
    tibble::tibble(
  ...
  # Shift fixture into CONUS bbox so the CONUS guard passes after
  ...
  # Output: a 20-col long tibble with carrier attribute.
  expect_s3_class(out, "tbl_df")
...
# ============================================================================
# T21-16 - weight_method = "population" fail-loud
#
...
# ============================================================================
# T21-17 - derived_rate carrier preservation
#
...
# rate itself is constructed by `cacs_derive_rates()` from these two
# count carriers). The carrier tibble must carry non-NA `est_total` +
# `var_total_raw` for the count so `cacs_derive_rates()` can read it as
# the rate numerator carrier.
#
...
test_that("T21-17 carrier preserves est_total/var_total_raw for spatial_total counts", {
  ...
  # This variable classifies as `spatial_total`. The carrier is what
  ...
  # Numerical preservation — cacs_derive_rates() will read est_total as the
  # rate numerator carrier.
...
# ============================================================================
# Fixtures + helpers - canonical EPSG:4326/4269 input pair builder.
#
...
# 2-tract synth at a CONUS-anchored origin (matches the integration
# fixture above) so the CONUS guard + EPSG:5070 round-trip both pass.
# ============================================================================
...
test_that("T21-18 composite cache hit returns byte-identical output on second call", {
  ...
  expect_true(any(grepl("Using the result saved in the cache folder", msgs, fixed = TRUE)))
...
# ============================================================================
# T21-CARRIER-CONTRACT - forward check for the carrier lookups
#
# The `cacs_aggregation_carriers` attribute MUST be byte-identical to what
# `.lookup_carrier_num/_den/_var()` read. We assert the contract here so a
# future rename of a carrier column (e.g. `est_total` -> `total_estimate`)
# trips the test BEFORE the lookups silently NULL-coalesce it.
#
# Required carrier schema:
...
# ============================================================================
# T22-09 .. T22-12 — C1 -> C2 multi-step fallback chain end-to-end
#
...
# from the zero_den / missing_moe scenarios (T22-11 /
...
#
# All four cases also assert the fallback priority via two adversarial
# rows in T22-11 / T22-12 where two triggers fire simultaneously and the
# higher-priority reason wins.
# ============================================================================

# Builder for a single-rate fixture. `formula = NULL` uses
# auto-dispatch (C2 default); pass `"proportion_subset"` to force C1.
...
test_that("T22-PRIORITY priority order: zero_den beats missing_moe and neg_var", {
  ...
  # AND num_var would force inside_negative if it weren't NA. Fallback
...
test_that("T22-NO-OP .apply_* helpers are no-ops on empty idx", {
  # Dispatch contract: each branch helper must be safely callable
...
test_that("T21-CARRIER-CONTRACT carrier attribute matches Sec. 22.3 step 6 lookup contract", {
  ...
  # ---- (5) Lookup simulation: the lookup helpers resolve numerator carrier
  ...
  # Carrier attach order matters: the attribute is set
...
# ============================================================================
# T23-08 .. T23-11 — cacs_derive_rates() known-answer fixtures
#
# Verifies cacs_derive_rates() at ε=1e-9 across the four MOE fallback
...
# poverty_rate / labor_force_participation -> proportion_subset,
...
# ============================================================================
# T23-CARRIER-DROP — consume-then-drop carrier
# ============================================================================
...
# ============================================================================
# T23-ROW-CARDINALITY — row cardinality contract.
# ============================================================================
