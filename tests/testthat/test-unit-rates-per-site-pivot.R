...
testthat::test_that("variant 2: n_sites = 50 perf-ish run does not crash", {
  ...
  # Soft perf budget: 50 sites x 5 rates is < 1 sec on any modern hardware. A
  # wall-clock budget depends on the load of the machine, so it is checked only
  # where NOT_CRAN is "true".
...
testthat::test_that("variant 2: duplicate key-rate rows abort", {
  ...
  testthat::expect_error(
    ...
    "more than one row for the same"
  )
})
