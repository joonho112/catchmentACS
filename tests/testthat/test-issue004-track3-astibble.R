...
#
# Track 3: as_tibble.cacs_run_result() rate-first sort.
# ===========================================================================
...
# --- rate_first = TRUE: rate rows first ------------------------------------
# Without the argument the rows keep their order (test-unit-astibble-row-order.R).

test_that("ISSUE004-T3-01 rate_first = TRUE: first 5 rows = AL_BHM_01 rate rows", {
  rr <- .rr_fixture()
  tbl <- tibble::as_tibble(rr, rate_first = TRUE)
...
test_that("ISSUE004-T3-02 rate_first = TRUE: rate rows come BEFORE B-codes within site", {
  rr <- .rr_fixture()
  tbl <- tibble::as_tibble(rr, rate_first = TRUE)
