# ============================================================================
# Unit tests for R/moe-helpers.R primitives.
# 15 cases + 3 robustness extras.
# ============================================================================
...
test_that("T-MOE-17 .classify_acs_variable covers all 14 default vars", {
  # cacs_acs_default_vars, including the SSI numerator added in v0.3.
...
# ============================================================================
# cacs_propagate_moe(): validation + Family A/B (vectorized).
# 12 cases T22-01..T22-08 + T22-13..T22-16.
# ============================================================================
...
# every test below can exercise the public entry without needing the
...
.make_phase6_fixture <- function(add_rate_row = FALSE) {
  ...
  if (isTRUE(add_rate_row)) {
    ...
    # carrier values are computed from num/den lookups, so only the key
    # tuple needs to exist.
...
test_that("T22-04 cacs_propagate_moe() rejects unsanctioned formula arguments", {
  ...
  # fallback_chain_max is accepted with a provenance warning.
  expect_warning(
...
test_that("T22-08 cacs_propagate_moe() dispatches ratio rows to C2 default (Step 6.3)", {
  ...
  # auto-dispatch produces `general_ratio_conservative` (the default)
