# ============================================================================
# Unit tests for cacs_run() pre-dispatch + boundary surface.
#
# Pre-dispatch in cacs_run() covers thin arg validation + execution
# path determination + sub-call arg-list validation + warning collector
# init. The 5-call orchestrator (with partial-success NA propagation) runs
# on top, and the output dispatch comes last — successful traversal
# lands on the long-format/list-column output surface. These
# tests exercise that boundary plus narrow argument-forwarding regressions:
#
#   T24-08 - thin validation success path returns the long tibble via
#            mocked sub-calls.
...
#            still succeeds and the call returns the long tibble).
...
# matter for the argument checks, which abort at the pre-dispatch
...
# fixture data, `cacs_run()` should traverse pre-dispatch + the 5 sub-calls
# cleanly and return the long tibble with its run-level attributes. We also
# expect the M-24-01 execution-path message to fire as a side effect.
#
...
test_that("T24-08 thin validation succeeds and returns a long-format tibble", {
  ...
  .with_run_stubs_local({
    ...
    # The 5-call path now returns a long tibble carrying the
...
test_that("T24-13 weight_method = 'population' aborts before provider work", {
  ...
  expect_match(conditionMessage(err), "No routing or ACS request was sent", fixed = TRUE)
})
...
# the warning fires, validation continues and the call returns the
...
test_that("T24-14 unknown moe_args key warns (W-24-01) and is dropped", {
  ...
  .with_run_stubs_local({
    # withCallingHandlers so we observe the warning. testthat::expect_warning()
    # with `class=` filters the warning by condition class; the trailing
    # `error = function(e) NULL` swallows any error so it does not surface
    # as a test failure.
    expect_warning(
      tryCatch(
        cacs_run(sites = sites, state = "AL",
  ...
  # early (before any sub-call), so no stubs are needed for the
...
#
# By design, `moe_args$formula` and
