# ============================================================================
# Unit tests for cacs_derive_rates() argument validation.
#
# Covers T23-01..T23-07:
...
test_that("T23-03 cacs_derive_rates aborts on unsanctioned custom rate (E-23-04)", {
  ...
  expect_s3_class(err, "catchmentACS_error_schema")
  # The message names the built-in rates and does not point to a function
  # that does not exist.
  expect_match(conditionMessage(err), "five built-in rates", fixed = TRUE)
  expect_false(grepl("cacs_derive_rates_custom", conditionMessage(err),
                     fixed = TRUE))
...
test_that("T23-05 cacs_derive_rates aborts when sanctioned rate code modified (E-23-04)", {
  ...
  expect_s3_class(err, "catchmentACS_error_schema")
  expect_match(conditionMessage(err), "cacs_acs_default_rates", fixed = TRUE)
  expect_match(conditionMessage(err), "ACS codes", fixed = TRUE)
})
...
test_that("T23-07b cacs_derive_rates aborts when critical column missing (E-23-02)", {
  fx <- .make_derive_args_fixture()
  fx$failure_origin <- NULL  # drop a required column of the MOE output
...
test_that("T23-DOTS-01 an argument given through ... is an error, not ignored", {
  fx <- .make_derive_args_fixture()
  err <- tryCatch(cacs_derive_rates(fx, level = 0.95), error = function(e) e)
  expect_s3_class(err, "catchmentACS_error_schema")
  expect_match(conditionMessage(err), "level", fixed = TRUE)
  expect_match(conditionMessage(err), "cacs_propagate_moe", fixed = TRUE)
  err_unnamed <- tryCatch(
    cacs_derive_rates(fx, cacs_acs_default_rates, "general_ratio_conservative",
                      TRUE, 0.95),
    error = function(e) e
  )
  expect_s3_class(err_unnamed, "catchmentACS_error_schema")
  # The arguments are not evaluated to build the message.
  err_lazy <- tryCatch(cacs_derive_rates(fx, level = stop("not evaluated")),
                       error = function(e) e)
  expect_s3_class(err_lazy, "catchmentACS_error_schema")
})
