# ============================================================================
# test-bug001-water-tract.R - v0.2.0 regression suite.
#
# Locks the BUG-001 hybrid fix. Covers BOTH layers:
#
...
#
# Fixture: `inst/testdata/fixture_water_tract_baldwin.rds`,
...
test_that("Testcase 3: intersect demotes degenerate abort to warn-and-skip", {
  ...
  out <- withCallingHandlers(
    ...
    # Silence the unrelated cross-state soft hint of cacs_intersect_weight()
...
# ============================================================================
# Testcase 4 - Canonical 22-mandatory long-output schema preserved
#              after auto-filter.
# ============================================================================
...
test_that("Testcase 6: all-degenerate input preserves fail-loud abort", {
  ...
  # Sanity: bbox is finite + CONUS-inside, but areas are zero. The areas are
  # measured in EPSG:5070, as cacs_intersect_weight() measures them: with s2
  # off, sf measures longitude/latitude only with the lwgeom package, which is
  # not installed with sf.
  expect_true(all(is.finite(sf::st_bbox(all_degen))))
  expect_true(all(suppressWarnings(
    as.numeric(sf::st_area(sf::st_transform(all_degen, 5070)))
  ) == 0))
...
# ============================================================================
# Testcase 11 - With s2 off, the zero-area check does not need lwgeom
# ============================================================================

test_that("Testcase 11: with s2 off, the zero-area check measures the tracts without lwgeom", {
  skip_if_no_water_fixture()
  fixture <- load_water_fixture()
  ...
  with_s2 <- catchmentACS:::.drop_water_tracts(
    fixture, drop_water_tracts = TRUE, verbose = FALSE
  )

  # A user can turn s2 off for the session. sf then measures longitude and
  # latitude with lwgeom, which is not installed with sf, so the check must
  # measure the tracts in a projection. The replacement of sf::st_area()
  # records what it is given and measures as sf does.
  suppressMessages(sf::sf_use_s2(FALSE))
  measured <- character()
  sf_area <- sf::st_area
  testthat::local_mocked_bindings(
    st_area = function(x, ...) {
      measured <<- c(measured, if (isTRUE(sf::st_is_longlat(x))) "longlat" else "projected")
      sf_area(x, ...)
    },
    .package = "sf"
  )
  without_s2 <- catchmentACS:::.drop_water_tracts(
    fixture, drop_water_tracts = TRUE, verbose = FALSE
  )

  expect_true(length(measured) > 0L)
  expect_false("longlat" %in% measured)
  expect_identical(without_s2$dropped_geoids, with_s2$dropped_geoids)
  expect_identical(without_s2$dropped_reasons, with_s2$dropped_reasons)
  expect_identical(without_s2$kept_sf$GEOID, with_s2$kept_sf$GEOID)
})
