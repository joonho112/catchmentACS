...
test_that("T-CACHE-OPT-09 global scope shows the line that keeps the cache off", {
  with_test_cache({
    ...
    expect_message(
      ...
      "options(catchmentACS.cache_enabled = FALSE)", fixed = TRUE
    )
...
test_that("T-CACHE-OPT-10 repeated global scope calls write no file", {
  with_test_cache({
    rp <- tempfile("Rprofile_")
    withr::local_options(catchmentACS.rprofile_path = rp)
    withr::local_envvar(R_PROFILE_USER = rp)
    suppressMessages(cacs_set_cache(FALSE, scope = "global", confirm = FALSE))
    suppressMessages(cacs_set_cache(FALSE, scope = "global", confirm = FALSE))
    expect_false(file.exists(rp))
    expect_false(cacs_get_cache_state()$enabled)
  })
...
test_that("T-CACHE-OPT-11 global scope turning the cache on shows the cache folder line", {
  with_test_cache({
    withr::local_options(width = 200, cli.width = 200)
    rp <- tempfile("Rprofile_")
    old_block <- c(
      "# catchmentACS v0.3 cache control",
      "options(catchmentACS.cache_enabled = FALSE)",
      "# end catchmentACS v0.3 cache control"
    )
    writeLines(old_block, rp)
    withr::local_options(catchmentACS.rprofile_path = rp)
    suppressMessages(cacs_set_cache(FALSE, scope = "global", confirm = FALSE))
    expect_message(
    ...
    expect_identical(readLines(rp, warn = FALSE), old_block)
