# ============================================================================
# Integration tests for the intersect -> MOE -> rates end-to-end chain.
#
# Drives the cacs_intersect_weight() -> cacs_propagate_moe() ->
# cacs_derive_rates() pipeline against a fixture that mirrors the
...
# cacs_intersect_weight() aggregation produces a carrier entry for every
# code referenced by .SANCTIONED_RATES_V1.
#
...
# CONUS-valid (-86.80, 33.50) so the CONUS guard + EPSG:5070 round-trip pass.
...
test_that("T23-INT-05 canonical 27-col schema snapshot (v0.3 topology column)", {
  ...
  #     `ring_topology` on top of the v0.2 22-col schema).
