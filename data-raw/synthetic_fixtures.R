# data-raw/synthetic_fixtures.R - writes seven small files in inst/testdata/
# for tests whose answer can be worked out by hand, to 1e-9.
#
# The shapes are simple and fixed: sf POINT and POLYGON objects in EPSG:5070,
# except `al_10_site_sample`, which stays in EPSG:4326 like
# `cacs_alabama_sites`.
#
# Run from the package root:
#   Rscript data-raw/synthetic_fixtures.R
#
# The margins of error of the two proportion fixtures are derived from the
# variances rather than given: num_moe = z * sqrt(A_var) and
# den_moe = z * sqrt(B_var), with z = 1.645. For the fixture with A = 50,
# A_var = 10, B = 200, B_var = 400, that keeps the value under the square root
# of the proportion formula negative (10 - 0.0625 * 400 = -15), which is the
# case that makes the package fall back to the ratio formula. A margin of error
# of 10 for that fixture would give A_var = (10 / 1.645)^2 = 36.95 and a value
# of about +11.95 instead, and the fallback would not be reached.
...
# estimate = c(100, 200, 300, 400), MOE = c(12, 18, 24, 30).
tracts_4 <- sf::st_sf(
...
# Fixture 4: the table of totals (4 rows x 8 cols) for rows T22-09 to T22-12
# The four rows cover the outcomes of the proportion formula:
...
# num_moe and den_moe are z * sqrt(A_var) and z * sqrt(B_var); see the note at
# the top of the file.
z <- 1.645
...
saveRDS(
  list(
    inputs = list(
      num_est = 50,
      num_moe = num_moe_T10,    # ~5.202 (see the note at the top of the file)
      den_est = 200,
      den_moe = den_moe_all,    # 32.90
      z       = z
    ),
...
# The bundled cacs_alabama_sites data written to inst/testdata/ as well, so
# that tests can read it with readRDS()
...
# The values the tests check, to 1e-9
...
# The value under the square root of the proportion formula
p <- 50 / 200
